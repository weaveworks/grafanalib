...
def stacked(graph):
  ...
  newGraph.update(dict(
    # Bit of a gotcha here. `graph` is a Python dictionary form of the Grafana
    # JSON object (i.e. an AST). In the Python DSL, we use consistent
    # camelCase naming, but Grafana itself is inconsistent. Thus, what was
    # specified as `lineWidth` when passed to Graph is overridden as
    # `linewidth` here.
    linewidth=0,
