...
DASHBOARD_LINK_ICON = Literal[
    "bolt", "cloud", "dashboard", "doc", "external link", "info", "question"
]
...
class ePictBox(object):
    ...
    orbLocation = attr.ib(
        default="Left", validator=in_(["Left", "Right", "Top", "Bottom"])
    )
...
def is_valid_max_per_row(instance, attribute, value):
    if (value is not None) and not isinstance(value, int):
        raise ValueError(
            "{attr} should either be None or an integer".format(attr=attribute)
        )
...
def is_valid_xaxis_mode(instance, attribute, value):
    ...
    if value not in XAXIS_MODES:
        raise ValueError(
            "{attr} should be one of {choice}".format(
...
class YAxes(object):
    ...
    left = attr.ib(
        default=attr.Factory(lambda: YAxis(format=SHORT_FORMAT)),
...
def to_y_axes(data):
    ...
    if not isinstance(data, (list, tuple)):
        raise ValueError(
            "Y axes must be either YAxes or a list of two values, got %r" % data
        )
    if len(data) != 2:
        raise ValueError(
            "Must specify exactly two YAxes, got %d: %r" % (len(data), data)
        )
    warnings.warn(
...
class DashboardLink(object):
    ...
    icon: DASHBOARD_LINK_ICON = attr.ib(
        default="external link", validator=in_(DASHBOARD_LINK_ICON.__args__)
    )
    ...
    type: DASHBOARD_TYPE = attr.ib(
        default="dashboards", validator=in_(DASHBOARD_TYPE.__args__)
    )
...
class Template(object):
    ...
    autoCount = attr.ib(default=DEFAULT_AUTO_COUNT, validator=instance_of(int))
...
class AlertCondition(object):
    ...
    timeRange = attr.ib(
        default=None,
        validator=attr.validators.optional(attr.validators.instance_of(TimeRange)),
    )
...
class AlertExpression(object):
    ...
    expressionType = attr.ib(
        ...
        validator=in_(
            [EXP_TYPE_CLASSIC, EXP_TYPE_REDUCE, EXP_TYPE_RESAMPLE, EXP_TYPE_MATH]
        ),
    )
    hide = attr.ib(default=False, validator=instance_of(bool))
...
def is_valid_triggers(instance, attribute, value):
    ...
    for trigger in value:
        if not isinstance(trigger, tuple):
            raise ValueError(
                f"{attribute.name} must be a list of [(Target, AlertCondition)] tuples"
            )
...
def is_valid_triggersv9(instance, attribute, value):
    ...
    for trigger in value:
        if not (isinstance(trigger, Target) or isinstance(trigger, AlertExpression)):
            raise ValueError(
                f"{attribute.name} must either be a Target or AlertExpression"
            )
...
class AlertRulev8(object):
    ...
    evaluateInterval = attr.ib(
        default=DEFAULT_ALERT_EVALUATE_INTERVAL, validator=instance_of(str)
    )
    evaluateFor = attr.ib(
        default=DEFAULT_ALERT_EVALUATE_FOR, validator=instance_of(str)
    )
...
class Panel(object):
    ...
    extraJson = attr.ib(
        default=None, validator=attr.validators.optional(instance_of(dict))
    )
...
class RowPanel(Panel):
    ...
    def to_json_data(self):
        return self.panel_json(
            {"collapsed": self.collapsed, "panels": self.panels, "type": ROW_TYPE}
        )
...
class TimeSeries(Panel):
    ...
    valueMin = attr.ib(
        default=None, validator=attr.validators.optional(instance_of(int))
    )
    valueMax = attr.ib(
        default=None, validator=attr.validators.optional(instance_of(int))
    )
    valueDecimals = attr.ib(
        default=None, validator=attr.validators.optional(instance_of(int))
    )
    axisSoftMin = attr.ib(
        default=None, validator=attr.validators.optional(instance_of(int))
...
class Discrete(Panel):
    ...
    backgroundColor = attr.ib(
        default=RGBA(128, 128, 128, 0.1), validator=instance_of((RGBA, RGB, str))
    )
    lineColor = attr.ib(
        default=RGBA(0, 0, 0, 0.1), validator=instance_of((RGBA, RGB, str))
    )
    metricNameColor = attr.ib(
        default="#000000", validator=instance_of((RGBA, RGB, str))
    )
    timeTextColor = attr.ib(default="#d8d9da", validator=instance_of((RGBA, RGB, str)))
    valueTextColor = attr.ib(default="#000000", validator=instance_of((RGBA, RGB, str)))
    ...
    legendSortBy = attr.ib(
        default="-ms", validator=in_(["-ms", "ms", "-count", "count"])
    )
...
class AlertList(object):
    ...
    gridPos = attr.ib(
        default=None,
        validator=attr.validators.optional(attr.validators.instance_of(GridPos)),
    )
    id = attr.ib(default=None)
...
class StatValueMappingItem(object):
    ...
    def to_json_data(self):
        return {
            self.mapValue: {"text": self.text, "color": self.color, "index": self.index}
        }
...
class StatRangeMapping(object):
    ...
    def to_json_data(self):
        return StatMapping(
            self.text, startValue=self.startValue, endValue=self.endValue, id=self.id
        )
...
class BarGauge(Panel):
    ...
    orientation = attr.ib(
        ...
        validator=in_([ORIENTATION_HORIZONTAL, ORIENTATION_VERTICAL, ORIENTATION_AUTO]),
    )
    rangeMaps = attr.ib(default=attr.Factory(list))
    thresholdLabels = attr.ib(default=False, validator=instance_of(bool))
    thresholdMarkers = attr.ib(default=True, validator=instance_of(bool))
    thresholds = attr.ib(
        default=attr.Factory(
            lambda: [Threshold("green", 0, 0.0), Threshold("red", 1, 80.0)]
        ),
...
class Heatmap(Panel):
    ...
    cards = attr.ib(default={"cardPadding": None, "cardRound": None})
    ...
    yAxis = attr.ib(default=attr.Factory(YAxis), validator=instance_of(YAxis))
...
class PieChart(Panel):
    ...
    def to_json_data(self):
        print(
            "PieChart panel was deprecated in Grafana 8.0, please use PieChartv2 instead"
        )
        return self.panel_json(
            {
                "aliasColors": self.aliasColors,
...
class SeriesOverride(object):
    ...
    dashLength = attr.ib(
        default=None, validator=attr.validators.in_([*range(1, 21), None])
    )
    spaceLength = attr.ib(
        default=None, validator=attr.validators.in_([*range(1, 21), None])
    )
...
class Worldmap(Panel):
    ...
    locationData = attr.ib(
        default="countries", validator=attr.validators.in_(WORLDMAP_LOCATION_DATA)
    )
    ...
    mapCenter = attr.ib(
        default="(0°, 0°)", validator=attr.validators.in_(WORLDMAP_CENTER)
    )
    ...
    thresholdColors = attr.ib(
        default=["#73BF69", "#73BF69", "#FADE2A", "#C4162A"],
        validator=instance_of(list),
    )
...
class BarChart(Panel):
    ...
    thresholdSteps = attr.ib(
        default=attr.Factory(
            lambda: [{"value": None, "color": "green"}, {"value": 80, "color": "red"}]
        ),
        validator=instance_of(list),
    )
    overrides = attr.ib(factory=list, validator=instance_of(list))
                ...
                "fieldConfig": {
                    "defaults": {
                        ...
                        "color": {
                            "mode": self.colorMode,
                            "fixedColor": (
                                self.fixedColor if self.colorMode == "fixed" else "none"
                            ),
                        },
                        "mappings": self.mappings,
                        "thresholds": {
